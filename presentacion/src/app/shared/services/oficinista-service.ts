...
export class OficinistaService {
  ...
  filtrar(parametros: any) {
    ...
    return this.http.get<any>(`${_SERVER}/api/oficinista/filtrar/0/5`, { params });
  }

  guardar(datos: TipoOficinista, id?: number): Observable<TipoOficinista> {
    delete datos.id;
    if (id) {
      return this.http.put<any>(`${_SERVER}/api/oficinista/${id}`, datos).pipe(
        catchError(this.handleError)
      );
    }
    return this.http.post<any>(`${_SERVER}/api/oficinista`, datos).pipe(
      catchError(this.handleError)
    );
  }

  eliminar(id: number) {
    return this.http.delete(`${_SERVER}/api/oficinista/${id}`, { 
      responseType: 'text',
      observe: 'response'
    }).pipe(
      map(response => {
  ...
  buscar(id: number) {
    return this.http.get<TipoOficinista>(`${_SERVER}/api/oficinista/${id}`).pipe(
      catchError(this.handleError)
    );
  }
