...
export class FrmAdministrador implements OnInit {
  titulo!: string;
  srvAdministrador = inject(AdministradorService); // Descomenta cuando tengas el servicio
    ...
    this.myForm = this.builder.group({
      ...
      apellido1: [
        ...
        [
          Validators.required,
          Validators.minLength(2),
          Validators.maxLength(15), // Administrador tiene límite de 15 en apellidos
          Validators.pattern('^[A-Za-zÑñáéíóúÁÉÍÓÚ ]+$'),
        ],
      ],
      apellido2: [
  ...
  onGuardar() {
    ...
    if (this.myForm.value.id === 0) {
      this.srvAdministrador.guardar(this.myForm.value).subscribe({
        next: () => {
          this.dialog.open(DialogoGeneral, {
            data: {
              texto: 'Administrador agregado correctamente',
              icono: 'check',
        ...
        error: (err) => {
          let mensaje = 'Error al agregar el administrador';
      ...
      this.srvAdministrador
        .guardar(this.myForm.value, this.myForm.value.id)
        .subscribe({
          next: () => {
            this.dialog.open(DialogoGeneral, {
              data: {
                texto: 'Administrador modificado correctamente',
                icono: 'check',
                textoAceptar: ' Aceptar ',
              },
            });
          ...
          error: (err) => {
            let mensaje = 'Error al modificar el administrador';
            
            if (err === 409) {
              mensaje = 'El ID de administrador o correo electrónico ya existe en el sistema';
            } else if (err === 400) {
              mensaje = 'Datos inválidos. Verifique la información ingresada';
            } else if (err === 404) {
